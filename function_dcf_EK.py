...
def dcf(lcA, lcB, t, dt):
    ...
    for k in range(t.shape[0]):
        ...
        #probando formula dcf original   ##################  
        #mts2 = np.mean(lcB[ts2idx,1])
        #mts1 = np.mean(lcA[ts1idx,1])
        mts2 = np.mean(lcA[:,1])
        mts1 = np.mean(lcB[:,1])
        n[k] = ts1idx.shape[0]
        #dcf_low = np.sqrt((np.var(lcA[ts1idx,1]) - np.mean(lcA[ts1idx,2])**2)  * (np.var(lcB[ts2idx,1]) - np.mean(lcB[ts2idx,2])**2))       
        dcf_low =  np.sqrt((np.var(lcA[:,1]) - np.mean(lcA[:,2])**2)  *      (np.var(lcB[:,1]) - np.mean(lcB[:,2])**2))
        #dcf_upper = (lcB[ts2idx,1] - mts2) * (lcA[ts1idx,1] - mts1) / dcf_low
        dcf_upper = (lcB[ts2idx,1] - mts2) * (lcA[ts1idx,1] - mts1) / dcf_low
        
        if n[k] > 0:
            dcf[k] = np.sum(dcf_upper) / float(n[k])
        else:
            dcf[k] = 0.0001  # Set dcf[k] to 0.0 when n[k] is 0
    
        if n[k] > 1:
            dcferr[k] = np.sqrt(np.sum((dcf_upper- dcf[k])**2)) / float(n[k] - 1)
        else:
            dcferr[k] = 0.0001  # Set dcferr[k] to 0.0001 when n[k] is 1
...
#
def find_dcf_delay(fuenteLead, fuenteDelayed, trudelay, prep_technique):
    ...
    for p in range(1):
        if 0.1 <= trueDelay <= 20.00:
            ...
            delta_min, delta_max, bandera_100_120, tope, range_low, range_high = 10, 120, 1, 32, 99.95, 121.05
    ...
    dcf_Results = dcf_Results.rename({0 :"DCF_Max", 1 :"DCFERR", 2 :"EstimatedDelay", 3 :"bin", 4 :"delta min", 5 :"delta max", 6 :"True delay", 7 :"prep_technique", 8 :"error"}, axis='columns')
    ...
    dcf_Results['DCF_Max'] = pd.to_numeric(dcf_Results['DCF_Max'])
    dcf_Results['DCFERR'] = pd.to_numeric(dcf_Results['DCFERR'])
    ...
    max_corelationIdx = dcf_Results["DCF_Max"].idxmax()
    ...
    'DCF_Max':[dcf_Results["DCF_Max"].mean()],'DCFERR':[dcf_Results["DCFERR"].mean()],'EstimatedDelay':[dcf_Results["EstimatedDelay"].mean()],
    ...
    'DCF_Max':[dcf_Results["DCF_Max"].mode().values[0]],'DCFERR':[dcf_Results["DCFERR"].mode().values[0]],'EstimatedDelay':[dcf_Results["EstimatedDelay"].mode().values[0]],
    ...
    dcf_Results['DCF_Max'] = dcf_Results['DCF_Max'].map('{:,.5f}'.format)
    dcf_Results['DCFERR'] = dcf_Results['DCFERR'].map('{:,.5f}'.format)
